import { Route as rootRoute } from "./routes/__root";
import { Route as indexRoute } from "./routes/index";
import { Route as loginRoute } from "./routes/login";
import { Route as registerRoute } from "./routes/register";
import { Route as dashboardIndexRoute } from "./routes/dashboard/index";
import { Route as dashboardLeadsRoute } from "./routes/dashboard/leads";
import { Route as dashboardCrmRoute } from "./routes/dashboard/crm";
import { Route as dashboardOnboardingRoute } from "./routes/dashboard/onboarding";
import { Route as dashboardSocialRoute } from "./routes/dashboard/social";
import { Route as empresaSlugRoute } from "./routes/empresa/$slug";
import { Route as dashboardServicesRoute } from "./routes/dashboard/services";
import { Route as dashboardFollowUpRoute } from "./routes/dashboard/follow-up";
import { Route as dashboardMessagesRoute } from "./routes/dashboard/messages";
import { Route as dashboardAutomationsRoute } from "./routes/dashboard/automations";
import { Route as dashboardBillingRoute } from "./routes/dashboard/billing";

const indexRouteWithParent = indexRoute.update({
  id: "/",
  path: "/",
  getParentRoute: () => rootRoute,
} as any);

const loginRouteWithParent = loginRoute.update({
  id: "/login",
  path: "/login",
  getParentRoute: () => rootRoute,
} as any);

const registerRouteWithParent = registerRoute.update({
  id: "/register",
  path: "/register",
  getParentRoute: () => rootRoute,
} as any);

const dashboardIndexRouteWithParent = dashboardIndexRoute.update({
  id: "/dashboard/",
  path: "/dashboard/",
  getParentRoute: () => rootRoute,
} as any);

const dashboardOnboardingRouteWithParent = dashboardOnboardingRoute.update({
  id: "/dashboard/onboarding",
  path: "/dashboard/onboarding",
  getParentRoute: () => rootRoute,
} as any);

const dashboardLeadsRouteWithParent = dashboardLeadsRoute.update({
  id: "/dashboard/leads",
  path: "/dashboard/leads",
  getParentRoute: () => rootRoute,
} as any);

const dashboardCrmRouteWithParent = dashboardCrmRoute.update({
  id: "/dashboard/crm",
  path: "/dashboard/crm",
  getParentRoute: () => rootRoute,
} as any);

const dashboardSocialRouteWithParent = dashboardSocialRoute.update({
  id: "/dashboard/social",
  path: "/dashboard/social",
  getParentRoute: () => rootRoute,
} as any);

const empresaSlugRouteWithParent = empresaSlugRoute.update({
  id: "/empresa/$slug",
  path: "/empresa/$slug",
  getParentRoute: () => rootRoute,
} as any);

const dashboardServicesRouteWithParent = dashboardServicesRoute.update({
  id: "/dashboard/services",
  path: "/dashboard/services",
  getParentRoute: () => rootRoute,
} as any);

const dashboardFollowUpRouteWithParent = dashboardFollowUpRoute.update({
  id: "/dashboard/follow-up",
  path: "/dashboard/follow-up",
  getParentRoute: () => rootRoute,
} as any);

const dashboardMessagesRouteWithParent = dashboardMessagesRoute.update({
  id: "/dashboard/messages",
  path: "/dashboard/messages",
  getParentRoute: () => rootRoute,
} as any);
const dashboardAutomationsRouteWithParent = dashboardAutomationsRoute.update({
  id: "/dashboard/automations",
  path: "/dashboard/automations",
  getParentRoute: () => rootRoute,
} as any);

const dashboardBillingRouteWithParent = dashboardBillingRoute.update({
  id: "/dashboard/billing",
  path: "/dashboard/billing",
  getParentRoute: () => rootRoute,
} as any);

export const routeTree = rootRoute.addChildren([
  indexRouteWithParent,
  loginRouteWithParent,
  registerRouteWithParent,
  dashboardIndexRouteWithParent,
  dashboardOnboardingRouteWithParent,
  dashboardLeadsRouteWithParent,
  dashboardCrmRouteWithParent,
  dashboardFollowUpRouteWithParent,
  dashboardMessagesRouteWithParent,
  dashboardAutomationsRouteWithParent,
  dashboardServicesRouteWithParent,
  dashboardSocialRouteWithParent,
  dashboardBillingRouteWithParent,
  empresaSlugRouteWithParent,
]);